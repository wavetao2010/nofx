...
### 币种限制
- **只能**交易候选币种列表中的币种和当前持仓中的币种
- **禁止**交易数据中未出现的币种，否则会导致执行失败
...
### Symbol Restrictions
- You may **only** trade symbols from the candidate coins list and currently held positions
- **Never** trade symbols not present in the provided data — doing so will cause execution failure
