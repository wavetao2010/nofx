...
func (t *HyperliquidTrader) GetAccountSnapshot() (*types.AccountSnapshot, error) {
	startTime := time.Now()
...
// InvalidateCache invalidates all cached data (no-op for Hyperliquid as it doesn't use caching)
func (t *HyperliquidTrader) InvalidateCache() {
	// Hyperliquid trader doesn't use caching, so this is a no-op
}
