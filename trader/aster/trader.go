...
func (t *AsterTrader) GetAccountSnapshot() (*types.AccountSnapshot, error) {
	startTime := time.Now()
...
// InvalidateCache invalidates all cached data (no-op for Aster as it doesn't use caching)
func (t *AsterTrader) InvalidateCache() {
	// Aster trader doesn't use caching, so this is a no-op
}
